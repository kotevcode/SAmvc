...
class Debug {
    ...
    {
        $args = func_get_args();
        self::$reporting && print(self::style().PHP_EOL.'<pre class="debug log">'.implode(
    ...
    {
        $args = func_get_args();
        self::$reporting && die(call_user_func_array('\SAmvc\Services\Debug::log', $args));
    }
    ...
    static public function chrono($print = null, $scope = '')
    {
        ...
        } elseif ($print && isset(self::$chrono[$scope]))
        {
            ...
            $chrono[] = '<div class="table"><b>'.$scope.' chrono table</b>'.PHP_EOL.sprintf(
                '%\'-61s %-46s<i>duration</i>%1$s%1$\'-61s',
                PHP_EOL,
                'unit - action'
              ).implode(PHP_EOL, $table).PHP_EOL.'</div>';
        }
    ...
    {
        if (self::$registered['shutdown'] && ($error = error_get_last(
          )) && ($error['type'] & (E_ERROR | E_PARSE | E_CORE_ERROR | E_COMPILE_ERROR))
        )
        {
    ...
    static public function handler($type, $message, $file, $line, $scope, $stack = null)
    {
        ...
        $stack = $stack ? $stack : array_slice(
          debug_backtrace(false),
          $type & E_USER_ERROR | E_USER_WARNING | E_USER_NOTICE ? 2 : 1
        ); // clean stack depending if error is user triggered or not
        self::overload($stack, $file, $line); // switch line & file if overloaded method triggered the error
        echo self::style(), PHP_EOL, '<pre class="debug error ', strtolower(
          self::$error[$type]
        ), '">', PHP_EOL, sprintf(
          '<b>%s</b>: %s in <b>%s</b> on line <b>%s</b>'  // print error
          ,
    ...
    static protected function overload(&$stack, &$file, &$line)
    {
        ...
        } // clean stack and overwrite file & line
    }
    ...
    static protected function context($stack, $scope)
    {
        ...
        {
            $context[] = sprintf(
              '  <span class="trace">#%s %s: <b>%s%s%s</b>(%s)</span>',
              $index,
    ...
    static protected function simple_export($var)
    {
        ...
        if (is_object($var))
        {
            return substr($export, 0, strpos($export, '#'));
        } // strstr( $export, '#', true );
